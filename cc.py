...
# Rate limiting semaphore để giới hạn số request đồng thời
_edit_message_semaphore = asyncio.Semaphore(3)  # Tối đa 3 request đồng thời
_last_edit_time = {}  # Track thời gian request cuối cùng cho mỗi chat

async def safe_edit_message_text(query_or_message, text, reply_markup=None, parse_mode=None, max_retries=3):
    """
    Safely edit message text với retry logic và rate limiting.
    Xử lý BadRequest, RetryAfter (429), và rate limiting.
    """
    # Lấy chat_id để track rate limit theo từng chat
    chat_id = None
    ...
    # Thêm delay nhỏ giữa các request cho cùng một chat (tối thiểu 0.1 giây)
    if chat_id:
        current_time = time.time()
        if chat_id in _last_edit_time:
            time_since_last = current_time - _last_edit_time[chat_id]
            if time_since_last < 0.1:  # Delay tối thiểu 100ms giữa các request
    ...
    async with _edit_message_semaphore:
        for attempt in range(max_retries):
            try:
                ...
                # Xử lý BadRequest (ví dụ: message not modified)
                if "message is not modified" in str(e).lower():
                    return  # Không cần retry cho lỗi này
                elif "message to edit not found" in str(e).lower():
                    logger.warning("Message to edit not found, skipping")
                    return
                else:
                    logger.error(f"BadRequest when editing message: {e}")
                    raise  # Re-raise các BadRequest khác
            except TimedOut:
                # Xử lý timeout
                wait_time = (attempt + 1) * 1.0  # Exponential backoff cho timeout
                ...
                if attempt < max_retries - 1:
                    ...
                    # Không log lại ở lần cuối để tránh spam log
                    pass
                return  # Không retry cho các lỗi không xác định
...
BOT_TOKEN = "8132478896:AAFN3dE5bpGv72f1AMLK35lVTVLllcVbyq0"
...
def get_user_record(user_id, username=None, first_name=None):
    ...
    # Cập nhật username và first_name nếu được cung cấp
    # Cập nhật ngay cả khi username là None (để đánh dấu đã kiểm tra)
    if username is not None and data.get('username') != username:
        data['username'] = username
        changed = True
    elif username is None and data.get('username') is None:
        # Nếu username là None và chưa có trong DB, đánh dấu là đã kiểm tra nhưng không có
        # Không cần thay đổi vì đã là None rồi
        pass
    if first_name is not None and data.get('first_name') != first_name:
        data['first_name'] = first_name
        changed = True
    # Chỉ tự động set VIP cho admin nếu ADMIN_USER_ID được set
...
def is_restricted_private(user_id, chat_id):
    ...
    """
    # Nếu là group chat (chat_id bắt đầu bằng "-"), không hạn chế
    if chat_id and str(chat_id).startswith("-"):
        return False
    
    # Nếu không phải chat riêng tư (chat_id không tồn tại hoặc None), không hạn chế
    if not chat_id:
        return False
    
    # Kiểm tra admin (chỉ khi ADMIN_USER_ID được set và không rỗng)
    ...
    # Chỉ VIP plan còn hiệu lực mới được phép ở chat riêng tư
    if plan == 'vip':
        # Kiểm tra thêm vip_expiry để đảm bảo VIP chưa hết hạn
        vip_expiry = user_data.get('vip_expiry')
        if vip_expiry:
            try:
                expiry_date = datetime.fromisoformat(vip_expiry)
                # Nếu VIP đã hết hạn, chặn user
                if datetime.now() > expiry_date:
                    return True  # VIP hết hạn, bị hạn chế
            except (ValueError, TypeError):
                # Nếu không parse được expiry date, coi như không có expiry (VIP vĩnh viễn)
                pass
        # VIP còn hiệu lực hoặc không có expiry (VIP vĩnh viễn)
        return False  # VIP không bị hạn chế
    
    # Tất cả các trường hợp khác (normal plan, chưa đăng ký, VIP hết hạn, etc.) đều bị hạn chế
    return True

def can_user_scan(user_id):
    user_data = get_user_record(user_id)
    # Kiểm tra admin (chỉ khi ADMIN_USER_ID được set)
    if ADMIN_USER_ID and str(user_id) == ADMIN_USER_ID:
        return True, ""
    # Kiểm tra VIP plan và expiry
    if user_data['plan'] == 'vip':
        vip_expiry = user_data.get('vip_expiry')
        if vip_expiry:
            try:
                expiry_date = datetime.fromisoformat(vip_expiry)
                # Nếu VIP đã hết hạn, không cho scan
                if datetime.now() > expiry_date:
...
def activate_key(key, user_id, username, first_name):
    ...
    # Lưu username và first_name khi activate key
...
async def show_start_login(update: Update = None, query=None):
    # Kiểm tra quyền truy cập chat riêng tư
    if update:
        user = update.effective_user
        chat = update.effective_chat
        chat_id = chat.id if chat else None
        
        # Không reply trong group chat
        if chat_id and str(chat_id).startswith("-"):
            return
    ...
    if user and chat_id is not None:
        if is_group_chat:
            ...
            if str(chat_id) not in ALLOWED_GROUP_CHAT_IDS and (not ADMIN_USER_ID or str(user.id) != ADMIN_USER_ID):
                ...
                return
            # Group chat được phép - hiển thị menu
            keyboard = [
...
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not is_registered(user.id):
        # Trong group chat được phép, vẫn hiển thị menu
        chat = update.effective_chat
        chat_id = chat.id if chat else None
        is_group_chat = chat_id and str(chat_id).startswith("-")
    ...
    # Trong group chat được phép, không kiểm tra restricted_private
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Lấy bot instance để có thể query Telegram API
    ...
    for uid, data in users_db.items():
        ...
        # Nếu không có username trong database, thử lấy từ Telegram API
        if not username and bot:
            user_id_int = int(uid)
            username_found = False
            
            # Phương pháp 1: Thử get_chat (chỉ hoạt động nếu bot đã tương tác với user)
            try:
            ...
            # Phương pháp 2: Thử lấy từ group chat nếu bot ở trong group với user
            if not username_found:
                try:
                    # Thử các group chat được phép
            ...
            # Phương pháp 3: Thử lấy từ admin group chat (nếu có)
            if not username_found:
                try:
                    # Thử lấy từ group chat hiện tại nếu đang ở trong group
...
async def admin_set_vip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Thử lấy username từ Telegram API nếu có thể
    username = None
    first_name = None
    if context.bot:
        try:
            ...
            # Không thể lấy username, tiếp tục với None
            pass
    
    # Cập nhật username nếu lấy được
...
async def admin_del_vip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Kiểm tra xem input là user_id hay username
    if target_input.startswith('@'):
        # Tìm user_id từ username
        target_id = find_user_by_username(target_input)
        if not target_id:
            # Thử lấy user_id từ Telegram API nếu không tìm thấy trong database
            try:
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Kiểm tra nếu là group chat - chỉ xử lý khi bot được mention, reply, hoặc user đã chọn service
    if is_group_chat:
        # Kiểm tra xem user đã chọn service hoặc mode chưa
        ...
        # Nếu user đã chọn service hoặc mode, cho phép xử lý text
        if not has_selected_service and not has_mode:
            # Chưa chọn service - chỉ xử lý nếu bot được mention
            message_text = update.message.text or ""
            bot_username = context.bot.username if context.bot else None
            is_mentioned = False
            
            if bot_username:
                # Kiểm tra mention: @bot_username hoặc /command@bot_username
                is_mentioned = f"@{bot_username}" in message_text or f"/start@{bot_username}" in message_text.lower()
            
            # Kiểm tra reply - nếu message reply vào bot
            ...
            # Nếu không được mention và không reply vào bot, bỏ qua
            if not is_mentioned and not is_reply_to_bot:
                return
    
    # Private chat hoặc được mention trong group
    if not is_registered(user.id):
        # Chỉ reply trong private chat, không reply trong group
    ...
    # Chỉ xử lý khi có mode phù hợp, không reply tin nhắn bừa bãi
    ...
    # Không có mode phù hợp, không reply để tránh spam
    return
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_group_chat:
        ...
        # Nếu user đã chọn service hoặc mode, cho phép xử lý file
        if not has_selected_service and not has_mode:
            # Chưa chọn service - chỉ xử lý nếu bot được mention hoặc reply vào bot
    ...
    if mode == 'hotmail_checker':
        ...
        for idx, (email, password, original_line) in enumerate(accounts, start=1):
            ...
            try:
                ...
                die_count += 1
                # Cập nhật status ngay cả khi có lỗi
                checked = idx
                filled = int(bar_length * checked / total)
                bar = "[" + "█" * filled + "░" * (bar_length - filled) + "]"
                percent = int(checked * 100 / total)
                ...
                # Cố gắng update message nếu có thể
                try:
                    view = context.user_data.get('hotmail_view', 'status')
                    if view == 'status':
                        text = (
